...
import { UserDeviceLogModel } from '../../models/user-device-log.model';

declare var jQuery: any;
@Component({
  moduleId: module.id,
  selector: 'app-user-device-log',
  templateUrl: 'user-device-log.component.html',
  styleUrls: ['user-device-log.component.css'],
  directives: [NgClass, NgIf, CORE_DIRECTIVES, FORM_DIRECTIVES, AjaxWaitingComponent]
})
export class UserDeviceLogComponent implements OnInit {
  ...
  //  Search
	...
	public userDeviceLogs: Array<UserDeviceLogModel> = [];
  ...
  ngOnInit() {
	  ...
	  jQuery('#page-selection').bootpag({
		  ...
		  jQuery.ajax({
			  url: _urlPrefix+'userDeviceLog/read',
			  data: params, 
			  type: "GET", 
			  dataType: 'json',
			  success: function(response) {
				  _this.userDeviceLogs = response.data;
			  }
		  });
	  });
  ...
  public onClickSearch(searchWord) {
	  ...
	  this._http.get(environment['urlPrefix']+'userDeviceLog/read', { search: params })
  ...
  readRes(response) {
	  this.userDeviceLogs = response.data;
